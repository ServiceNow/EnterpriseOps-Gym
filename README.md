...
```bash
docker pull <gym-image>
docker run -d -p <port>:<port> <gym-image>
```
...
Update `conf/ray/domain_conf.json` if you use non-default ports.
...
Task success rate (%) on Oracle mode. A task passes only if **all** verification conditions are met.
